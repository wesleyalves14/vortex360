...
export default function TestimonialsSection() {
  return (
    <section
      id="parceiros"
      aria-label="A voz dos nossos parceiros"
      className="relative isolate bg-transparent py-10 sm:py-12 md:py-16 lg:py-20 touch-pan-y overscroll-y-contain"
    >
      <div className="mx-auto w-full max-w-7xl px-6 md:px-10">
        ...
        <div className="relative border border-border/40 rounded-lg flex h-[28rem] w-full max-w-[1100px] overflow-hidden gap-x-2 sm:gap-x-3 lg:gap-x-4 bg-transparent contain-content touch-pan-y overscroll-y-contain min-h-[220px] sm:min-h-[260px] transform-gpu will-change-[transform] [backface-visibility:hidden]">
          <div className="flex flex-row items-center gap-x-3 sm:gap-x-4 lg:gap-x-5 transform-gpu will-change-[transform] [backface-visibility:hidden]">
            <Marquee vertical pauseOnHover loop={false} className="[--duration:135s]">
              {depoimentos.map((r) => <DepoimentoCard key={r.username} {...r} />)}
            </Marquee>
            <Marquee vertical pauseOnHover reverse loop={false} className="[--duration:135s]">
              {depoimentos.map((r) => <DepoimentoCard key={r.username} {...r} />)}
            </Marquee>
            <Marquee vertical pauseOnHover loop={false} className="[--duration:135s]">
              {depoimentos.map((r) => <DepoimentoCard key={r.username} {...r} />)}
